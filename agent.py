...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing==True:
            ...
            #epsilon=self.epsilon-0.05
            #self.epsilon=epsilon
            
            #t=self.trial_number+1
            #self.trial_number=t
            # 1/a^t decay function
            #t=self.env.step_data['t']
            #if t==0:
            #    self.epsilon=1
            #else:
            #    self.epsilon=1/(t**2)
            #epsilon=self.epsilon*self.init_epsilon
            #self.epsilon=epsilon

            a_t=self.at+self.a
            self.at=a_t
            
            epsilon=math.cos(a_t)
    ...
    def choose_action(self, state):
        ...
        if self.learning==True: # choose a random action
            
            randNum=random.uniform(0,1)  # generate a random number between 0,1
            if randNum>self.epsilon:
                # choose action with highest Q-value for the current state
                actionsDict=self.Q[str(state)]
                #print("current action:",action,"   actionsDict:",actionsDict)
                #print("valid actions:",self.env.valid_actions)
                #print("self.Q",self.Q)
                #currentVal=actionsDict[str(action)]
                currentVal=actionsDict[action]
